...
fn is_truthy(obj: Lazy<Object>) -> bool {
    match obj.as_ref() {
...
impl<'a> Evaluator {
    ...
    fn visit_update_assignment(
        ...
        match expr {
            ast::Expression::Identifier(exp) => {
                env.as_ref()
                    .borrow_mut()
                    .update(exp.value, obj.try_value()?)
                    .map(|_| self.runtime.NULL().into())
            }
    ...
    fn eval_infix_expression(&'a self, op: &str, left: Lazy<Object>, right: Lazy<Object>) -> EvalResult<'a> {
    ...
    fn visit_array_expression(&'a self, exp: ast::ArrayLiteral, env: Env) -> EvalResult<'a> {
        self.visit_expressions(exp.elements, env).map(|elements| {
            Object::ArrayLiteral(elements.into_iter().map(|e| e.to_owned()).collect()).into()
        })
    }
    ...
    fn apply_function(
        ...
        match func {
            Object::Function(parameters, body, func_env) => {
                let env =
    ...
    fn visit_call_expression(&'a self, expr: ast::CallExpression, env: Env) -> EvalResult<'a> {
        ...
        let block = expr.block
            ...
            .map(|o| o.to_owned())
            .or(env.borrow_mut().block().clone());

        self.apply_function(
            function.to_owned(),
            args.into_iter().map(|e| e.to_owned()).collect(),
            block,
            env,
        )
    }
    ...
    fn visit_statements(&'a self, stmts: Vec<ast::Node>, env: Env) -> EvalResult<'a> {
        ...
        for stmt in stmts.into_iter() {
            if let ast::Node::Statement(s) = stmt {
                ...
                match value {
                    r @ Lazy::Val(Object::Return(_)) => return Ok(Lazy::from(r)),
                    v => result = v.into(),
                }
            }
        }
    ...
    fn visit_let_statement(&'a self, stmt: ast::LetStatement, env: Env) -> EvalResult<'a> {
        ...
        let value = self.visit_expr(*stmt.value, env.clone())?.try_value()?;
    ...
    pub fn visit_statement(&'a self, stmt: ast::Statement, env: Env) -> EvalResult<'a> {
        ...
        match stmt {
            ...
            Return(ret) => {
                let result = self.visit_expr(*ret.value, env)?.try_value()?;
                match result {
                    Object::Return(r) => Ok(Lazy::Val(Object::Return(r))),
                    _ => Ok(Lazy::Val(Object::Return(Box::new(result)))),
                }
            }
